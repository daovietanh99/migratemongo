    ...
    if (isElementRangeSelection) {
      ...
      if ($isDecoratorNode(this) && this.isInline() && parentNode) {
        ...
        if (anchor.isBefore(focus)) {
          const anchorNode = anchor.getNode();
          const isAnchorPointToLast = anchor.offset === anchorNode.getChildrenSize();
          ...
          const focusNode = focus.getNode();
          const isFocusPointToLast = focus.offset === focusNode.getChildrenSize();
...
function getActiveEditorState() {
  if (activeEditorState === null) {
    {
      throw Error(`Unable to find an active editor state. State helpers or node methods can only be used synchronously during the callback of editor.update(), editor.read(), or editorState.read().`);
    }
  }
...
class ElementNode extends LexicalNode {
  ...
  append(...nodesToAppend) {
    // console.log("append", nodesToAppend, this.getChildrenSize())
...
class EditorState {
  ...
  exportJSON() {
    return readEditorState(null, this, () => {
        let root = exportNodeToJSON($getRoot())
        return {
            "root": root
        }
    });
...
class Lexical {
    constructor() {
    this.$addUpdateTag = $addUpdateTag;
    this.$applyNodeReplacement = $applyNodeReplacement;
    this.$cloneWithProperties = $cloneWithProperties;
    this.$copyNode = $copyNode;
    this.$createLineBreakNode = $createLineBreakNode;
    this.$createNodeSelection = $createNodeSelection;
    this.$createParagraphNode = $createParagraphNode;
    this.$createPoint = $createPoint;
    this.$createRangeSelection = $createRangeSelection;
    this.$createRangeSelectionFromDom = $createRangeSelectionFromDom;
    this.$createTabNode = $createTabNode;
    this.$createTextNode = $createTextNode;
    this.$getAdjacentNode = $getAdjacentNode;
    this.$getCharacterOffsets = $getCharacterOffsets;
    this.$getEditor = $getEditor;
    this.$getNearestNodeFromDOMNode = $getNearestNodeFromDOMNode;
    this.$getNearestRootOrShadowRoot = $getNearestRootOrShadowRoot;
    this.$getNodeByKey = $getNodeByKey;
    this.$getNodeByKeyOrThrow = $getNodeByKeyOrThrow;
    this.$getPreviousSelection = $getPreviousSelection;
    this.$getRoot = $getRoot;
    this.$getSelection = $getSelection;
    this.$getTextContent = $getTextContent;
    this.$hasAncestor = $hasAncestor;
    this.$hasUpdateTag = $hasUpdateTag;
    this.$insertNodes = $insertNodes;
    this.$isBlockElementNode = $isBlockElementNode;
    this.$isDecoratorNode = $isDecoratorNode;
    this.$isElementNode = $isElementNode;
    this.$isInlineElementOrDecoratorNode = $isInlineElementOrDecoratorNode;
    this.$isLeafNode = $isLeafNode;
    this.$isLineBreakNode = $isLineBreakNode;
    this.$isNodeSelection = $isNodeSelection;
    this.$isParagraphNode = $isParagraphNode;
    this.$isRangeSelection = $isRangeSelection;
    this.$isRootNode = $isRootNode;
    this.$isRootOrShadowRoot = $isRootOrShadowRoot;
    this.$isTabNode = $isTabNode;
    this.$isTextNode = $isTextNode;
    this.$isTokenOrSegmented = $isTokenOrSegmented;
    this.$nodesOfType = $nodesOfType;
    this.$normalizeSelection__EXPERIMENTAL = $normalizeSelection;
    this.$parseSerializedNode = $parseSerializedNode;
    this.$selectAll = $selectAll;
    this.$setCompositionKey = $setCompositionKey;
    this.$setSelection = $setSelection;
    this.$splitNode = $splitNode;
    this.ArtificialNode__DO_NOT_USE = ArtificialNode__DO_NOT_USE;
    this.BLUR_COMMAND = BLUR_COMMAND;
    this.CAN_REDO_COMMAND = CAN_REDO_COMMAND;
    this.CAN_UNDO_COMMAND = CAN_UNDO_COMMAND;
    this.CLEAR_EDITOR_COMMAND = CLEAR_EDITOR_COMMAND;
    this.CLEAR_HISTORY_COMMAND = CLEAR_HISTORY_COMMAND;
    this.CLICK_COMMAND = CLICK_COMMAND;
    this.COMMAND_PRIORITY_CRITICAL = COMMAND_PRIORITY_CRITICAL;
    this.COMMAND_PRIORITY_EDITOR = COMMAND_PRIORITY_EDITOR;
    this.COMMAND_PRIORITY_HIGH = COMMAND_PRIORITY_HIGH;
    this.COMMAND_PRIORITY_LOW = COMMAND_PRIORITY_LOW;
    this.COMMAND_PRIORITY_NORMAL = COMMAND_PRIORITY_NORMAL;
    this.CONTROLLED_TEXT_INSERTION_COMMAND = CONTROLLED_TEXT_INSERTION_COMMAND;
    this.COPY_COMMAND = COPY_COMMAND;
    this.CUT_COMMAND = CUT_COMMAND;
    this.DELETE_CHARACTER_COMMAND = DELETE_CHARACTER_COMMAND;
    this.DELETE_LINE_COMMAND = DELETE_LINE_COMMAND;
    this.DELETE_WORD_COMMAND = DELETE_WORD_COMMAND;
    this.DRAGEND_COMMAND = DRAGEND_COMMAND;
    this.DRAGOVER_COMMAND = DRAGOVER_COMMAND;
    this.DRAGSTART_COMMAND = DRAGSTART_COMMAND;
    this.DROP_COMMAND = DROP_COMMAND;
    this.DecoratorNode = DecoratorNode;
    this.ElementNode = ElementNode;
    this.FOCUS_COMMAND = FOCUS_COMMAND;
    this.FORMAT_ELEMENT_COMMAND = FORMAT_ELEMENT_COMMAND;
    this.FORMAT_TEXT_COMMAND = FORMAT_TEXT_COMMAND;
    this.INDENT_CONTENT_COMMAND = INDENT_CONTENT_COMMAND;
    this.INSERT_LINE_BREAK_COMMAND = INSERT_LINE_BREAK_COMMAND;
    this.INSERT_PARAGRAPH_COMMAND = INSERT_PARAGRAPH_COMMAND;
    this.INSERT_TAB_COMMAND = INSERT_TAB_COMMAND;
    this.IS_ALL_FORMATTING = IS_ALL_FORMATTING;
    ...
    this.KEY_ARROW_DOWN_COMMAND = KEY_ARROW_DOWN_COMMAND;
    this.KEY_ARROW_LEFT_COMMAND = KEY_ARROW_LEFT_COMMAND;
    this.KEY_ARROW_RIGHT_COMMAND = KEY_ARROW_RIGHT_COMMAND;
    this.KEY_ARROW_UP_COMMAND = KEY_ARROW_UP_COMMAND;
    this.KEY_BACKSPACE_COMMAND = KEY_BACKSPACE_COMMAND;
    this.KEY_DELETE_COMMAND = KEY_DELETE_COMMAND;
    this.KEY_DOWN_COMMAND = KEY_DOWN_COMMAND;
    this.KEY_ENTER_COMMAND = KEY_ENTER_COMMAND;
    this.KEY_ESCAPE_COMMAND = KEY_ESCAPE_COMMAND;
    this.KEY_MODIFIER_COMMAND = KEY_MODIFIER_COMMAND;
    this.KEY_SPACE_COMMAND = KEY_SPACE_COMMAND;
    this.KEY_TAB_COMMAND = KEY_TAB_COMMAND;
    this.LineBreakNode = LineBreakNode;
    this.MOVE_TO_END = MOVE_TO_END;
    this.MOVE_TO_START = MOVE_TO_START;
    this.OUTDENT_CONTENT_COMMAND = OUTDENT_CONTENT_COMMAND;
    this.PASTE_COMMAND = PASTE_COMMAND;
    this.ParagraphNode = ParagraphNode;
    this.REDO_COMMAND = REDO_COMMAND;
    this.REMOVE_TEXT_COMMAND = REMOVE_TEXT_COMMAND;
    this.RootNode = RootNode;
    this.SELECTION_CHANGE_COMMAND = SELECTION_CHANGE_COMMAND;
    this.SELECTION_INSERT_CLIPBOARD_NODES_COMMAND = SELECTION_INSERT_CLIPBOARD_NODES_COMMAND;
    this.SELECT_ALL_COMMAND = SELECT_ALL_COMMAND;
    this.TEXT_TYPE_TO_FORMAT = TEXT_TYPE_TO_FORMAT;
    this.TabNode = TabNode;
    this.TextNode = TextNode;
    this.UNDO_COMMAND = UNDO_COMMAND;
    this.createCommand = createCommand;
    this.createEditor = createEditor;
    this.getNearestEditorFromDOMNode = getNearestEditorFromDOMNode;
    this.isBlockDomNode = isBlockDomNode;
    this.isCurrentlyReadOnlyMode = isCurrentlyReadOnlyMode;
    this.isHTMLAnchorElement = isHTMLAnchorElement;
    this.isHTMLElement = isHTMLElement;
    this.isInlineDomNode = isInlineDomNode;
    this.isSelectionCapturedInDecoratorInput = isSelectionCapturedInDecoratorInput;
    this.isSelectionWithinEditor = isSelectionWithinEditor;
    this.resetRandomKey = resetRandomKey;
    }
}
